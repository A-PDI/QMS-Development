...
function buildDimensionalFromTests(tests) {
  ...
  for (const t of tests) {
    if (isFlushStep(t)) continue;        // internal bench diagnostic, not a test point
    ...
    answers.push({
      ...
      spec,
      // Measured average, or "Error: <description>" when the step errored.
      actual1: stepResultValue(t, p),
      status: statusVal,
      __unit: p.unit || '',
      __error: err.errored ? err.description : '',
    });
    ...
    if (t.secondary) {
      ...
      const secondaryLabel = stepLabel(t, 'secondary', s.tank_name);
      items.push({
        id,
        measurement: secondaryLabel === primaryLabel
          ? `${secondaryLabel} — ${s.tank_name || 'Secondary'}`
          : secondaryLabel,
        spec: s.spec || '',
      });
      answers.push({
 ...
 * @param {Object} opts          { actor } — the user who asked for the report;
 *                               defaults to the bench when generated by a job.
 * Returns true if a NEW inspection was created.
 */
function autoFillReportInspection(reportExtId, injectorRows, opts = {}) {
  ...
  const actorName = (opts.actor && opts.actor.name) || 'Injector Test Bench';
  ...
  try {
    db.run(
      `INSERT INTO inspection_activity_log (id, inspection_id, action_type, actor_name, actor_id, created_at)
       VALUES (?, ?, 'started', ?, ?, ?)`,
      [crypto.randomUUID(), inspectionId, actorName, actorId, now]
    );
  } catch (_) {}
