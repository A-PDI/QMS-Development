...
export default function NCRDetail() {
  ...
  if (!isNew && isLoading) return <div className="p-6 text-gray-400">Loading…</div>
    ...
    <div className="min-h-full bg-gray-50/50">
      ...
      <div className="sticky top-0 z-10 bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <span className="font-bold text-pdi-navy text-base">
            {isNew ? 'New NCR' : ncr?.ncr_number}
          </span>
          {!isNew && ncr?.part_number && (
            <><span className="text-gray-400">·</span><span className="text-sm text-gray-600">{ncr.part_number}</span></>
          )}
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => navigate('/ncrs')}
            className="flex items-center gap-1.5 px-3 py-1.5 text-sm border border-gray-200 rounded-lg hover:bg-gray-50 active:bg-gray-100">
            <X size={14} /> Close
          </button>
          {!isNew && ncr?.status !== 'closed' && (
            <button onClick={handleClose} disabled={saving}
              className="flex items-center gap-1.5 px-3 py-1.5 text-sm bg-green-600 text-white rounded-lg hover:bg-green-700 active:bg-green-800 disabled:opacity-50">
              <CheckCircle size={14} /> Mark Closed
            </button>
          )}
          <button onClick={handleSave} disabled={saving}
            className="flex items-center gap-1.5 px-3 py-1.5 text-sm bg-orange-500 text-white rounded-lg hover:bg-orange-600 active:bg-orange-700 disabled:opacity-50">
      ...
      <div className="max-w-[900px] mx-auto p-6 space-y-5">
        ...
        {isNew && (
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-base font-semibold text-pdi-navy flex items-center gap-2">
            ...
            {linkedInspId && linkedInspection ? (
              <div className="space-y-3">
                <div className="flex items-center justify-between bg-orange-50 border border-orange-200 rounded-lg px-4 py-3">
                  <div>
                    <div className="text-sm font-semibold text-pdi-navy">{linkedInspection.form_no}</div>
                  ...
                  <button onClick={clearInspection} className="text-xs text-gray-400 hover:text-red-500">
                    <X size={14} />
                  </button>
                </div>
                      ...
                      {failedItems.map((item, i) => (
                        <div key={i} className="flex items-start gap-3 bg-red-50 border border-red-100 rounded-lg px-3 py-2">
                          ...
                          <div className="min-w-0">
                            ...
                            {item.notes && <div className="text-xs text-gray-500 mt-0.5 truncate">{item.notes}</div>}
                          </div>
                          <button
                            type="button"
                            ...
                            className="ml-auto text-xs text-pdi-navy hover:underline whitespace-nowrap flex-shrink-0"
                          >
                            Add to description
                          </button>
                        </div>
              ...
              <div className="space-y-3">
                <form onSubmit={e => { e.preventDefault(); setInspSearchActive(inspSearch) }} className="flex gap-2">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      className="w-full pl-8 pr-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-pdi-navy"
                    />
                  </div>
                  <button type="submit" className="px-3 py-1.5 text-sm bg-pdi-navy text-white rounded-lg hover:bg-pdi-navy-light">Search</button>
                  <button type="button" onClick={() => setShowInspSelector(false)} className="px-3 py-1.5 text-sm border border-gray-200 rounded-lg hover:bg-gray-50">
                    Skip
                  </button>
                </form>
                {inspList.length > 0 && (
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => setShowInspSelector(true)} className="text-xs text-pdi-navy hover:underline">Link an inspection</button>
              </div>
            )}
          </div>
        )}
        ...
        {!isNew && ncr && (
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="grid grid-cols-3 gap-4 text-sm">
              <div><div className="text-xs text-gray-400 mb-0.5">NCR Number</div><div className="font-bold text-pdi-navy font-mono">{ncr.ncr_number}</div></div>
              <div><div className="text-xs text-gray-400 mb-0.5">Created By</div><div>{ncr.created_by_name || '—'}</div></div>
              <div><div className="text-xs text-gray-400 mb-0.5">Created</div><div>{formatDateTime(ncr.created_at)}</div></div>
              {ncr.inspection_id && (
                <div>
                  <div className="text-xs text-gray-400 mb-0.5">Linked Inspection</div>
                  <button onClick={() => navigate(`/inspections/${ncr.inspection_id}`)} className="text-pdi-navy hover:underline text-sm">
                    {ncr.form_no || ncr.inspection_id}
                  </button>
                </div>
              )}
              {ncr.closed_at && (
                <div><div className="text-xs text-gray-400 mb-0.5">Closed</div><div>{formatDateTime(ncr.closed_at)}</div></div>
              )}
            </div>
          </div>
        )}
        ...
        {/* Part / PO Info */}
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <h3 className="text-base font-semibold text-pdi-navy mb-4">Part &amp; Supplier Information</h3>
        ...
        {/* Defect Details */}
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <h3 className="text-base font-semibold text-pdi-navy mb-4">Defect Details</h3>
          <div className="mb-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <h3 className="text-base font-semibold text-pdi-navy mb-4">Disposition &amp; Corrective Action</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
