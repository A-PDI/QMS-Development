...
const TABS = [
  { id: 'forms',   label: 'Inspection Forms', icon: FileText },
  { id: 'specs',   label: 'Part Specifications', icon: Package },
  { id: 'data',    label: 'Inspection Data', icon: Database },
]
...
function InspectionFormsTab({ showToast }) {
  ...
  if (isLoading) return <div className="p-6 text-gray-400">Loading templates…</div>

  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <button onClick={startCreate} className="flex items-center gap-1.5 px-4 py-2 text-sm bg-pdi-navy text-white rounded-lg hover:bg-pdi-navy-light">
          <Plus size={15} /> New Template
        </button>
      </div>

      {isEditing && (
        <div className="bg-white rounded-xl border border-pdi-navy/30 p-6 space-y-5 shadow-sm">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-semibold text-pdi-navy">{creating ? 'Create Template' : 'Edit Template'}</h3>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={addSection} className="flex items-center gap-1 text-xs text-pdi-navy hover:underline"><Plus size={13} /> Add Section</button>
            </div>
                  ...
                  <div key={sKey} className="border border-gray-200 rounded-lg overflow-hidden">
                    <div className="flex items-center justify-between px-4 py-2.5 bg-gray-50 cursor-pointer"
                      ...
                      <div className="flex items-center gap-3">
                        <input value={section.title} onChange={e => { e.stopPropagation(); updateSection(sKey, 'title', e.target.value) }}
                          ...
                          className="text-sm font-medium bg-transparent border-b border-dashed border-gray-300 focus:outline-none focus:border-pdi-navy w-60" />
                      ...
                      <div className="flex items-center gap-2">
                        <button onClick={e => { e.stopPropagation(); removeSection(sKey) }} className="text-red-400 hover:text-red-600 p-1"><Trash2 size={13} /></button>
                    ...
                    {open && (
                      <div className="p-3">
                        ...
                        <button onClick={() => addItem(sKey)} className="mt-2 text-xs text-pdi-navy hover:underline flex items-center gap-1"><Plus size={12} /> Add item</button>
                      </div>
                    )}
                  </div>
                )
          ...
          <div className="flex justify-end gap-3 pt-2 border-t border-gray-100">
            <button onClick={cancelEdit} className="px-4 py-2 text-sm border border-gray-200 rounded-lg hover:bg-gray-50">Cancel</button>
            <button onClick={creating ? handleCreateTemplate : handleSaveTemplate}
              className="flex items-center gap-1.5 px-4 py-2 text-sm bg-pdi-navy text-white rounded-lg hover:bg-pdi-navy-light">
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-4">
        <select value={selectedTemplateId} onChange={e => { setSelectedTemplateId(e.target.value); setEditingSpec(null) }}
          className="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-pdi-navy">
      ...
      {!selectedTemplateId && (
        <div className="text-center text-gray-400 py-12 text-sm">Select an inspection form to manage part specifications</div>
      )}

      {selectedTemplateId && editingSpec && (
        <div className="bg-white rounded-xl border border-pdi-navy/30 p-5 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-semibold text-pdi-navy">{editingSpec === 'new' ? 'New Part Specification' : 'Edit Part Specification'}</h3>
            <button onClick={() => setEditingSpec(null)}><X size={18} className="text-gray-400 hover:text-gray-600" /></button>
          </div>
                ...
                {dimItems.map(({ sKey, item }) => {
                  ...
                  return (
                    <div key={specKey} className="flex items-center gap-3">
                      <span className="text-xs text-gray-600 w-56 truncate">{item.measurement || item.name || `Item ${item.id}`}</span>
                      <input
                        ...
                        className="flex-1 border border-gray-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-pdi-navy"
                      />
                    </div>
                  )
                })}
          ...
          <div className="flex justify-end gap-3 pt-2 border-t border-gray-100">
            <button onClick={() => setEditingSpec(null)} className="px-4 py-2 text-sm border border-gray-200 rounded-lg hover:bg-gray-50">Cancel</button>
            <button onClick={handleSave} className="flex items-center gap-1.5 px-4 py-2 text-sm bg-pdi-navy text-white rounded-lg hover:bg-pdi-navy-light">
              <Save size={14} /> Save
            </button>
          </div>
        </div>
      ...
      {selectedTemplateId && !editingSpec && (
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          {isLoading ? <div className="text-center text-gray-400 py-10">Loading…</div> :
            specs.length === 0 ? <div className="text-center text-gray-400 py-10">No part specifications for this form type</div> : (
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
        ...
        <form onSubmit={applySearch} className="flex gap-2">
          <div className="relative">
            ...
            <input type="text" placeholder="Part #, PO, inspector, lot…"
              className="pl-8 pr-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-pdi-navy w-56"
              value={search} onChange={e => setSearch(e.target.value)} />
          </div>
          <button type="submit" className="px-3 py-1.5 text-sm bg-pdi-navy text-white rounded-lg hover:bg-pdi-navy-light">Search</button>
        </form>
        <select className="px-3 py-1.5 text-sm border border-gray-200 rounded-lg" value={filters.status || ''} onChange={e => setFilter('status', e.target.value)}>
          <option value="">All Statuses</option>
          <option value="draft">Open</option>
          <option value="complete">Complete</option>
        </select>
        <select className="px-3 py-1.5 text-sm border border-gray-200 rounded-lg" value={filters.component_type || ''} onChange={e => setFilter('component_type', e.target.value)}>
          <option value="">All Components</option>
          {Object.entries(COMPONENT_TYPE_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
        </select>
        <input type="date" className="px-3 py-1.5 text-sm border border-gray-200 rounded-lg" value={filters.date_from || ''} onChange={e => setFilter('date_from', e.target.value)} />
        <span className="text-gray-400 text-sm self-center">to</span>
        <input type="date" className="px-3 py-1.5 text-sm border border-gray-200 rounded-lg" value={filters.date_to || ''} onChange={e => setFilter('date_to', e.target.value)} />
        <span className="text-xs text-gray-500 self-center ml-auto">{total} records</span>
      </div>
  ...
  return (
    <div className="min-h-full bg-gray-50/50">
      <div className="bg-white border-b border-gray-200 px-6 py-5">
        <div className="flex items-center gap-3">
          <Settings size={22} className="text-pdi-navy" />
          <h1 className="text-2xl font-bold text-pdi-navy">Admin</h1>
        </div>
      </div>
